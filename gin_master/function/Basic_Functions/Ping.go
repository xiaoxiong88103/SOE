...
import (
	...
	"influxdb/gin_master/function/Basic_Functions/bf_ini"
	"net/http"
	"time"
)
...
func Ping_node(c *gin.Context) {
	...
	count, err := bf_ini.Get_config_int("ping", "count")
	if err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": err})
		return
	}
	timeout, err := bf_ini.Get_config_int("ping", "timeout")
	if err != nil {
		c.JSON(http.StatusForbidden, gin.H{"error": err})
		return
	}

	pinger.Count = count                                  // 发送ping的次数
	pinger.Timeout = time.Second * time.Duration(timeout) // 超时时间
